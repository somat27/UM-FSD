...
def registar_produtor_certificado(nome_produtor, public_key):
    ...
    response = requests.post(f"http://{IP_Gestor}:{Porta_Gestor}/produtor_certificado", json=post_data)
    if response.status_code in (200, 201):
        certificado_pem = response.text
        return certificado_pem
    else:
        raise Exception("Erro ao registrar produtor no gestor.")
...
def menu_inicial():
    global Info_Produtor, private_key, public_key, certificado
    while True:
        private_key, public_key = gerar_chaves()
        nome_produtor = input("Digite o nome do produtor: ")
        certificado = registar_produtor_certificado(nome_produtor, public_key)
